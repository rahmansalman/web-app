...
//if access code is valid send to apply.php
if (array_key_exists('submitted', $_POST) && $_POST['submitted'] == 1)
{
    ...
    if (is_array(checkAccessCode($access_code))){
        redirect("apply", "access_code=$access_code");
    }
    redirect("login", "failed");
}
...
if (array_key_exists('failed', $_GET)){
    $error_msg = "Used/Invalid Access code supplied.";
}
...
<section class="bg-gray-50 min-h-screen px-6 py-6 md:px-20 md-20">
    ...
    <div class="flex items-center flex-row flex-wrap justify-between h-full" style="min-height: 80vh">
        ...
        <div class="w-1/2">
            ...
            <div class="max-w-md flex flex-col">
                <h3 class="text-center font-bold text-3xl pb-4">Login</h3>
                <div class="border rounded w-full h-56 px-6 py-2 flex items-center">

                    <form action="<?=htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post" class="w-full" autocomplete="off">
                    <div class="">
                        <?php
                        if (!empty($error_msg)):
                            ?>
